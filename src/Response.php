...
class Response
{
    ...
    public function send($body): Response
    {
        ...
        if ($body) {
            switch (true) {
                case is_string($body):
                    //get shortcode in body
                    $body = $this->app->router->shortcodes->doShortcode($body);
                    if (!$this->get('Content-Type')) {
                        $this->type('html');
                    }
                    break;
    ...
    * handles that this script depends on. These dependencies will be loaded before the script
    * specified by the "handle" parameter.
    */
    public function enqueueScript($handle, $src, $deps = []){
        $this->app->enqueueScript($handle, $src, $deps);
    }
     ...
     * used to access the localized data in the JavaScript code.
     * @param data The "data" parameter is an array that contains the data you want to pass to the
     * localized script. This data can be accessed in the script using the "objectName" as a global
     * variable.
     */
    public function localizeScript($handle, $objectName, $data){
        $this->app->localizeScript($handle, $objectName, $data);
    }
}